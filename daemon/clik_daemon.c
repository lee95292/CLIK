	...
	while(1)
	{
		...
		switch(pid=fork()){
			...
			case 0:	
				...
				printf("client accepted IP:%s request message:%s\n\n",ipaddr,buf);
				tmpbuf=strtok(buf," ");
				...
				if(strcmp(tmpbuf,"request")==0){
					...
					sprintf(hostname,"%s",tmpbuf);
				        sprintf(cmdbuf,"~/CLIK/daemon/response.sh %s",ipaddr);
					...
					//sprintf(cmdbuf,"~/CLIK/daemon/processResult.sh %s %s",ipaddr,tmpbuf);
